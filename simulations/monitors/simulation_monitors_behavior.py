...
def single_run():
    ...
    if visual_output:
        main_window = qg.GraphicsWindow(title='Simulation: Behavioral Monitors')
...
if __name__ == '__main__':
    number_of_runs = 100
    behavior = {'reward': [], 'escape_latency': [], 'response': [], 'trajectory': []}
    for run in range(number_of_runs):
        print('Run: ', (run + 1))
        R, L, B, T = single_run()
        behavior['reward'].append(R)
        behavior['escape_latency'].append(L)
        behavior['response'].append(B)
        behavior['trajectory'].append(T)
    os.makedirs('data/', exist_ok=True)
    pickle.dump(behavior, open('data/behavior.pkl', 'wb'))
